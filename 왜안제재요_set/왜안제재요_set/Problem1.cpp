...
void InputLogFile(vector< InputData*> & inputData, map< string, int> * playerIdToInt, string * playerIntToId, set< string> * playerIdSet){
	FILE * fin;
	errno_t err;
	...
	for (auto i = playerIdSet->begin(); i != playerIdSet->end(); ++i){
		string s = *i;
		playerIdToInt->insert(map< string, int>::value_type(s, temp));
...
void DataPartition(vector< InputData*> & inputData, map< string, int> * playerIdToInt, string * playerIntToId, set< string> * playerIdSet, map< string, DataPart> * dataCover){
	...
	for (i = 0; i < dataCount; ++i){
		...
		if (j == dataCount){
			...
			dataCover->insert(map< string, DataPart>::value_type(inputData[i]->GetPlayerId(), data));
			break;
		}
		i = dungeonEndPos - 1;
		data.endPos = i;
...
void CountAdjPlayer(vector< InputData*> & inputData, map< string, set< int>> & settingInputData, map< string, int> & playerIdToInt, string * playerIntToId, set < string > & playerIdSet, map< string, DataPart> & dataCover){
...
void SettingData(vector< InputData*> & inputData, map< string, set< int>> & settingInputData, vector< set< int>*> & saveSetPointer, set< string> & playerIdset){
	unsigned int playerCount = playerIdset.size();
	for (auto& i = playerIdset.begin(); i != playerIdset.end(); ++i){
		string curPlayer = *i;
		set< int> * setArr = new set < int > ;
		saveSetPointer.push_back(setArr);
		settingInputData.insert(map < string, set< int> >::value_type(curPlayer, *setArr));
	}
...
int main(void){
	...
	map<string, set<int>> settingInputData;
	vector<set<int>*> saveSetPointer;
	inputData.reserve(5000001);
	saveSetPointer.reserve(512);
	inputData.reserve(INPUT_FILE_MAX);
	map< string, int> playerIdToInt;
	map< string, DataPart> dataCover;
