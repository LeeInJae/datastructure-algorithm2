...
void HeapSort(heap_t * maxHeap){
	int temp = maxHeap->size;
	for (int i = maxHeap->size; i >= 2; --i){
		Swap(&maxHeap->element[1], &maxHeap->element[i]);
		--maxHeap->size;
...
int HeapExtractMaximum(heap_t * maxHeap){
	if (maxHeap->size < 1) return -INF;

	int max = maxHeap->element[1];
	maxHeap->element[1] = maxHeap->element[maxHeap->size];
	--maxHeap->size;
	MaxHeapify(maxHeap, 1);
	return max;
}

void HeapIncrease(heap_t * maxHeap, int index, int data){
	maxHeap->element[index] = data;

	while (index>1 && maxHeap->element[index / 2] < data){
		Swap(&maxHeap->element[index], &maxHeap->element[index / 2]);
		index = index / 2;
	}
}

void HeapInsertion(heap_t * maxHeap, int data){
	++maxHeap->size;
	maxHeap->element[maxHeap->size] = -INF;
	HeapIncrease(maxHeap, maxHeap->size, data);
}
...
int main(void){
	...
	printf("최대값 추출후 제거 %d\n", HeapExtractMaximum(maxHeap));
	PrintHeap(maxHeap);

	HeapInsertion(maxHeap, 100);
	PrintHeap(maxHeap);
